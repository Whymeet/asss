...
class PrintCalcComponent extends CBitrixComponent implements Controllerable
{
    ...
    public function __construct($component = null)
    {
        ...
        require_once dirname(__FILE__) . '/lib/EmailFormatter.php';
        require_once dirname(__FILE__) . '/lib/EmailSender.php';
    }
                         ...
                         $numbering = false, $calcType = 'list', $calendarType = '', $pages = 14)
    {
        ...
        try {
            ...
            switch ($calcType) {
                ...
                case 'avtoviz':
                    $result = $this->calculateAvtoviz($paperType, $size, $quantity, $printType, $bigovka, $cornerRadius, $perforation, $drill, $numbering);
    ...
    private function calculateNote($paperType, $size, $quantity, $printType, $bigovka, $cornerRadius, $perforation, $drill, $numbering)
    {
        ...
        try {
            ...
            if ($backParams['printType'] === 'none') {
                $backResult = ['base' => ['totalPrice' => 0], 'additional' => 0, 'total' => 0];
            } else {
    ...
    public function sendOrderAction($name = '', $phone = '', $email = '', $callTime = '', $orderData = '')
    {
        ...
        try {
            ...
            $message = EmailFormatter::formatOrderMessage($orderInfo, $name, $phone, $email, $callTime, '');
            
            // Отправляем email
            $debugFn = function($msg, $data = null) { $this->debug($msg, $data); };
            $success = EmailSender::send($message, $orderInfo, $name, $phone, $email, $debugFn);
    ...
    public function sendOrderEmailAction($clientName = '', $clientPhone = '', $clientEmail = '', $callDate = '', $callTime = '', $clientComment = '', $orderDetails = '')
    {
        ...
        try {
            ...
            // Формируем сообщение для email
            $message = EmailFormatter::formatOrderMessage($orderInfo, $clientName, $clientPhone, $clientEmail, $preferredCallTime, $clientComment);
            
            // Отправляем email
            $debugFn = function($msg, $data = null) { $this->debug($msg, $data); };
            $success = EmailSender::send($message, $orderInfo, $clientName, $clientPhone, $clientEmail, $debugFn);
